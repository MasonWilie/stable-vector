...
constexpr int ceil_to_power_of_two(const int original, const int current_bit = 0)
{
    constexpr const int MAX_POWER = std::numeric_limits<int>::max()
        - (std::numeric_limits<int>::max() >> 1);
    const int current_power = 1 << current_bit;

    return (current_power >= original) || current_power == MAX_POWER
        ? current_power
        : ceil_to_power_of_two(original, current_bit + 1);
}
...
class stable_vector
{
 ...
 private:
    static constexpr const int EFFECTIVE_BLOCK_SIZE = ceil_to_power_of_two(BLOCK_SIZE);

    using storage_t = std::aligned_storage_t<EFFECTIVE_BLOCK_SIZE, alignof(value_type)>;
...
template <typename T, int BLOCK_SIZE>
inline auto stable_vector<T, BLOCK_SIZE>::max_size() const noexcept -> size_type
{
    return std::numeric_limits<size_type>::max();
}

template <typename T, int BLOCK_SIZE>
inline auto stable_vector<T, BLOCK_SIZE>::capacity() const noexcept -> size_type
{
...
template <typename T, int BLOCK_SIZE>
auto stable_vector<T, BLOCK_SIZE>::at(const size_type index) const -> const_reference
{
    validate_index_bounds(index);
    return (*this)[index];
}
...
template <typename T, int BLOCK_SIZE>
auto stable_vector<T, BLOCK_SIZE>::operator[](const size_type index) -> reference
{
...
template <typename T, int BLOCK_SIZE>
auto stable_vector<T, BLOCK_SIZE>::operator[](const size_type index) const -> const_reference
{
    const auto block_index = calculate_block_index(index);
    const auto block_offset = calculate_block_offset(index);
...
template <typename T, int BLOCK_SIZE>
void stable_vector<T, BLOCK_SIZE>::validate_index_bounds(const size_type index) const
{
    if (index >= _size)
    {
...
{
    if (_size == capacity()) _blocks.emplace_back(block_ptr_t(new block_t));
